...
export default function OstatkaEntry() {
    ...
    useEffect(() => {
        const fetchAll = async () => {
            ...
            try {
                const iRes = await fetch('http://localhost:5000/api/inventory', { signal: AbortSignal.timeout(3000) });
                const inventoryData = await iRes.json();

                const hRes = await fetch('http://localhost:5000/api/history', { signal: AbortSignal.timeout(3000) });
    ...
    const submitEntry = () => {
        ...
        fetch('http://localhost:5000/api/inventory/command', {
            method: 'POST',
